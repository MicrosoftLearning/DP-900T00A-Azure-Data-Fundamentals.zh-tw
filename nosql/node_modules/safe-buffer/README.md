...
`size` 必須小於或等於 `require('buffer').kMaxLength` 的值 (在 64 位元架構上，`kMaxLength` 為 `(2^31)-1`)， 否則會擲回 [`RangeError`][]。 如指定小於或等於 0 的 `size`，會建立長度為零的緩衝區。
...
***但如果使用 `Number` 引數呼叫 `toHex`，會發生什麼事？***
