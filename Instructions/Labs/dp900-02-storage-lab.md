...
1. 在 Azure 入口網站 首頁上，選取 **&#65291;從左上角建立資源**，並搜尋 `Storage account`。 然後在產生的 [儲存體帳戶]**** 頁面中，選取 [建立****]。
...
1. 在 [上傳 Blob****] 面板中，選取您先前儲存在本機電腦上的 **product1.json** 檔案。 然後在 [進階 **] 區段中的 [** 上傳至資料夾 **] 方塊中**，輸入`product_data`並選取 [**上傳]** 按鈕。
...
1. [從 下載product2.json](https://aka.ms/product2.json?azure-portal=true) JSON 檔案`https://aka.ms/product2.json`，並將它儲存在先前下載**product1.json**的相同資料夾中，稍後您將上傳至 Blob 記憶體。
...
1. 在 [檔案共享] 頁面中，選取 **&#65291;檔案共用**，並使用交易優化**層新增名為 `files` **的新檔案共用。
2. 選取 **[下一步：備份>** 並停用備份。 然後，選取 [檢閱 + 建立]****。
...
1. 在 [ **數據表]** 頁面上，選取 **&#65291;數據表** 並建立名為 `products`的新數據表。
