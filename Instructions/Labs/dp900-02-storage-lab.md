...
1. 在 [上傳 Blob****] 面板中，選取您先前儲存在本機電腦上的 **product1.json** 檔案。 然後在 進階 區**段的 上傳至資料夾** 方塊中**，輸入`product_data`並選取 **上傳**** 按鈕。
